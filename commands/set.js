...
exports.run = async (client, message, [action, key, ...value], level) => { // eslint-disable-line no-unused-vars
  ...
  if (action === "edit") {
    ...
    const exampleEmbed = new Discord.RichEmbed()
      .setColor('#0099ff')
      .setTitle('Edited settings!')
      .setDescription(`${key} successfully edited to ${value.join(" ")}`)
      .setThumbnail(message.author.avatarUrl)
      .setAuthor("Bot!", message.author.displayAvatar)
      .setTimestamp()
  ...
  if (action === "del" || action === "reset") {
    ...
    if (["y", "yes"].includes(response)) {
      ...
      .setColor('#0099ff')
      .setTitle('Reseting a setting!')
      .setDescription(`${key} was successfully deleted, and has been reset to default.`)
      .setThumbnail(message.author.avatarUrl)
      .setAuthor("Bot!", message.author.displayAvatar)
      .setTimestamp()
  ...
  if (action === "get") {
    ...
    const exampleEmbed2 = new Discord.RichEmbed()
      .setColor('#0099ff')
      .setTitle('Veiwing a setting!')
      .setDescription(`The value of ${key} is currently ${settings[key]}`)
      .setThumbnail(message.author.avatarUrl)
      .setAuthor("Bot!", message.author.displayAvatar)
      .setTimestamp()
    message.channel.send(exampleEmbed2);
  ...
  // Otherwise, the default action is to return the whole configuration in JSON format (to be prettified!);
    ...
    const exampleEmbed3 = new Discord.RichEmbed()
      ...
      .setDescription(`***__Current Guild Settings__***\n\`\`\`json\n${inspect(settings)}\n\`\`\``)
      .setAuthor("Bot!", message.author.displayAvatar)
      .addBlankField()
      .addField('Dashboard', `See the Dashboard on <${client.config.dashboard.callbackURL.split("/").slice(0, -1).join("/")}>`)
      .setTimestamp()
